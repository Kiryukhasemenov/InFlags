...
india.train_vocab('path/to/training/data', 'path/to/vocab.json')
```

If you already have a pre-trained vocabulary, you can import it at the instantiation of the InCa class: 

```aiignore
india = InDia(pretrained_vocab=True, vocab_file='path/to/vocab.json')
```
...
If you trained the vocabulary on Czech FLORES data, then "Švédsko" would be in the dictionary with such a diacritization; and "zemí" would not. So the output would be as follows:
